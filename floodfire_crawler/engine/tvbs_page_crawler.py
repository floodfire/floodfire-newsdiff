...
class TVBSPageCrawler(BasePageCrawler):
    ...
    def fetch_news_content(self, soup):
        ...
        # --- 取出標題 ---
        page['title'] = soup.find('meta', {'name': 'title'})['content']

        # --- 取出內文 ---
        for t in soup.select('script'):
            t.decompose()
        page['body'] = soup.find('div', id='news_detail_div').get_text(strip=True).replace('&nbsp', '\n')
        ...
        page['visual_contents'] = [{
            'type': 1,
            'visual_src': i['src'],
            'caption': i['alt'] if 'alt' in i else None
        }for i in images]

        # --- 取出影片數 ---
        video = soup.find('div', class_='video')
        if video is not None:
            video = video.div.p.iframe
            page['video'] = 1
            # -- 取出視覺資料連結（影片） ---
