...
class NtkPageCrawler(BasePageCrawler):
    ...
    def fetch_news_content(self, soup):
        ...
        # --- 取出內文 ---
        page["body"] = "".join(
            [p.text for p in soup.find("div", {"itemprop": "articleBody"}).findAll("p")]
        )
        ...
        # keywords=
        page["keywords"] = [
            a.text for a in soup.find("div", class_="tag_group2").findAll("a")
        ]
        ...
        page["visual_contents"] = [
            {"type": 1, "visual_src": "http:" + i.img["src"], "caption": i.img["alt"]}
            for i in image
        ]
