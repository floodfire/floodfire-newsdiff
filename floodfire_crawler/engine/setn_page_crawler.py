...
class SetnPageCrawler(BasePageCrawler):
    ...
    def fetch_news_content(self, soup):
        ...
        # --- 取出標題 ---
        page["title"] = (
            soup.find("h1", class_="news-title-3")
            or soup.find("h1", {"id": "newsTitle"})
        ).get_text(strip=True)
        ...
        page["body"] = "\n".join(
            [
                ...
                for p in soup.find("article").select("p")[
                    0
                    if soup.find("span", class_="reporter") != None
                    and soup.find("span", class_="reporter").get_text(strip=True) != ""
                    else 1 :
                ]
        ...
        page["keywords"] = [
            ...
            for a in soup.find("div", class_="keyword").findAll("strong")
        ]

        # --- 取出記者 ---
        # authors
        page["authors"] = [
            soup.find("span", class_="reporter").get_text(strip=True)
            if soup.find("span", class_="reporter") != None
            and soup.find("span", class_="reporter").get_text(strip=True) != ""
            else soup.find("article").find_all("p")[0].get_text(strip=True)
        ]
        ...
        page["visual_contents"] = [
            {"type": 1, "visual_src": i["src"], "caption": "None"} for i in image
        ]

        # --- 取出影片數 ---

        video = soup.find("iframe", {"id": "vodIframe"})
        if video:
            page["video"] = 1
            # -- 取出視覺資料連結（影片） ---
            page["visual_contents"].append(
                {
                    "type": 2,
                    "visual_src": "https://www.setn.com/" + video["src"],
                    "caption": None,
                }
            )
        else:
    ...
    def fetch_publish_time(self, soup):
        time = (
            soup.find("time", "page-date") or soup.find("div", class_="time")
        ).get_text(strip=True)
        news_time = strftime("%Y-%m-%d %H:%M:%S", strptime(time, "%Y/%m/%d %H:%M:%S"))
        return news_time
