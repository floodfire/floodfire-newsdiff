...
class UdnPageCrawler(BasePageCrawler):
    ...
    def fetch_news_content(self, soup):
        myjson_str = [
            ...
            if x.has_attr("type") and x["type"] == "application/ld+json"
        ][0]
        myjson_str_list = (
            myjson_str.text.replace("\r", "").replace("\t", "").split("\n")
        )
        content_list = []
        # 去除註解
        is_outside = True
        ...
        for video in video_list:
            page["visual_contents"].append(
                {"type": 2, "visual_src": video["url"], "caption": video["title"]}
            )
        ...
        if "window.location.href" in content_area[0].text:
            ...
            contents = [
                x.text
                for x in content_area
                if x.text != ""
                and x.text != " "
                and x.text != "\n"
                and x.p is None
                and x.script is None
            ]
            # 如果內文只有一張圖
            if len(contents) == 0:
                page["body"] = ""
            ...
            try:
                ...
                if status_code == requests.codes.ok:
                    ...
                    news_page["publish_time"] = str(
                        datetime.strptime(
                            news_page["publish_time"][:16], "%Y-%m-%d %H:%M"
                        )
                    )
                    ######Diff#######
                    version = 1
