...
class TsmPageCrawler(BasePageCrawler):
    ...
    def fetch_news_content(self, soup):
        ...
        # --- 取出標題 ---
        page["title"] = (
            soup.find(attrs={"id": "article_title"}).text.strip().replace("\u3000", " ")
        )
        # --- 取出內文 ---
        ...
        for x in article_content:
            if x.name == "p":
                ...
                # twitter欄位
                body_list.append(
                    "（# 水火標記：引用twitter文章，網址：" + x.find_all("a")[-1]["href"] + "）"
                )
        ...
        if head_video is not None:
            ...
            page["visual_contents"].append(
                {"type": 2, "visual_src": "https:" + video["src"], "caption": None}
            )
        ...
        if head_img is not None:
            img = head_img.img
            page["visual_contents"].append(
                {"type": 1, "visual_src": img["src"], "caption": img["alt"]}
            )
        ...
        for video in videos:
            if "facebook.com" in video["src"]:
                ...
                continue
            page["visual_contents"].append(
                {"type": 2, "visual_src": video["src"], "caption": None}
            )
        ...
        for row in crawl_list:
            try:
                ...
                if status_code == requests.codes.ok:
                    ...
                    news_page["image"] = len(
                        [v for v in news_page["visual_contents"] if v["type"] == 1]
                    )
                    news_page["video"] = len(
                        [v for v in news_page["visual_contents"] if v["type"] == 2]
                    )
