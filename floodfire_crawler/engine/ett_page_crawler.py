...
class EttPageCrawler(BasePageCrawler):
    ...
    def fetch_news_content(self, soup):
        ...
        # --- 取出標題 ---
        page['title'] = soup.find_all('h1')[-1].text.replace(
            "　",
            " "
        ).replace("\u200b", "")
        ...
        page['body'] = ('\n').join(
            [
                x for x in pure_story
                if len([p for p in ['▲', '▼', '文／'] if p in x]) == 0 and len(x) > 0
        ...
        # --- 取出圖片數 ---
        pic_urls_index = [
            i for i in range(0, len(story)) if story[i].img != None
        ]
        pic_list = list()
        for i in pic_urls_index:
            if((i-1) >= 0 and story[i-1].text.find('▼') >= 0):
                pic_list.append(
                    {'url': "https:"+story[i].img['src'],
                        'desc': story[i-1].text}
                )
