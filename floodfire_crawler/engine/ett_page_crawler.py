...
class EttPageCrawler(BasePageCrawler):
    ...
    def fetch_news_content(self, soup):
        ...
        # --- 取出標題 ---
        page["title"] = (
            soup.find_all("h1")[-1].text.replace("　", " ").replace("\u200b", "")
        )
        ...
        page["body"] = ("\n").join(
            [
                x
                for x in pure_story
                if len([p for p in ["▲", "▼", "文／"] if p in x]) == 0
                and len(x) > 0
        ...
        tags = (
            []
            if soup.find("div", {"class", "tag"}) == None
            else [x.text for x in soup.find("div", {"class", "tag"}).select("a")]
        )
        tags2 = (
            []
            if soup.find("p", {"class", "tag"}) == None
        ...
        authors = [
            author[0 : author.find("／")].replace(" ", "").replace("記者", "")
            if author.find("報導") > 0
            else author[(author.find("／") + 1) :].replace(" ", "").replace("記者", "")
        ...
        for img in pic_list:
            page["visual_contents"].append(
                {"type": 1, "visual_src": img["url"], "caption": img["desc"]}
            )
        ...
        for video in video_list:
            page["visual_contents"].append(
                {"type": 2, "visual_src": video["url"], "caption": video["desc"]}
            )
        return page
