...
class ApdPageCrawler(BasePageCrawler):
    ...
    def fetch_news_content(self, soup):
        ...
        # --- 取出發布時間 ---
        report["publish_time"] = self.fetch_publish_time(script_json["datePublished"])
        ...
        for other_image in other_images:
            image = {
                ...
                "caption": other_image.parent.parent.find(
    ...
    def fetch_publish_time(self, timeString):
        ...
        news_time = strftime(
            "%Y-%m-%d %H:%M:%S", strptime(timeString, "%Y-%m-%dT%H:%M:%S+08:00")
        )
        return news_time
