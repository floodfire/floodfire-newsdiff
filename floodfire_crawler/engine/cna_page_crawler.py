...
class CnaPageCrawler(BasePageCrawler):
    ...
    def fetch_news_content(self, soup):
        ...
        # --- 取出內文 ---
        content = "\n".join(
            [x.text for x in all_article.find("div", class_="paragraph").find_all("p")]
        )
        ...
        # keywords
        page["keywords"] = [
            x.text for x in all_article.find_all("div", class_="keywordTag")
        ]
        # --- 取出記者 ---
        # authors
        get_author = list()
        ...
        for i in range(len(image_list)):
            ...
            page["visual_contents"].append(
                {
                    "type": 1,
                    "visual_src": img_content["data-src"]
                    if "data-src" in str(img_content)
                    else img_content["src"],
                    "caption": caption,
                }
            )
            ...
            page["visual_contents"].append(
                {
                    ...
                    .text
                    if len(video_list[i].find_all("div", {"class": "picinfo"})) > 0
                    else "",
                }
            )
        ...
        for i in range(len(outerMedia_list)):
            if "data-href" in str(outerMedia_list[i]):
                ...
                page["video"] += 1
                page["visual_contents"].append(
                    {"type": 2, "visual_src": url_tag["href"], "caption": ""}
                )
