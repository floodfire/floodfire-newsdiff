...
class NowPageCrawler(BasePageCrawler):
    ...
    def fetch_news_content(self, soup):
        ...
        page["body"] = "\n".join(
            [
                p.text
                for p in soup.find("span", {"itemprop": "articleBody"}).findAll("p")
            ]
        )
        ...
        # keywords=
        page["keywords"] = [
            a.text for a in soup.find("div", class_="td-post-source-tags").findAll("a")
        ]
        ...
        page["visual_contents"] = [
            {
                "type": 1,
                "visual_src": i.img["src"]
                if i.noscript is None
                else i.noscript.img["src"],
        ...
        if len(soup.find("span", {"itemprop": "articleBody"}).findAll("p")) > 0:
            ...
            if (
                video is not None
                and video.noscript is not None
                and video.noscript.iframe is not None
            ):
                    ...
                    page["visual_contents"].append(
                        {
                            ...
                            if video.iframe.has_attr("src")
                            else video.iframe["data-src"],
                            "caption": None,
                        }
                    )
                else:
