...
class FloodfireStorage:
    ...
    def update_list_crawlercount(self, url_hash):
        ...
        """
        sql = (
            "UPDATE `list` SET `crawler_count` = `crawler_count` + 1 WHERE `url_md5`=%s"
        )
        try:
            self.cur.execute(sql, (url_hash,))
            self.conn.commit()
    ...
    def update_list_versioncount(self, url_hash):
        ...
        """
        sql = (
            "UPDATE `list` SET `version_count` = `version_count` + 1 WHERE `url_md5`=%s"
        )
        try:
            self.cur.execute(sql, (url_hash,))
            self.conn.commit()
    ...
    def get_crawllist(self, source_id, page_diff=False, diff_obj=None):
        ...
        if page_diff and diff_obj is not None:
            ...
            yesterday_time = (datetime.now() + timedelta(-1)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            try:
                self.cur.execute(
                ...
                if self.cur.rowcount > 0:
                    ...
                    rs = [
                        x
                        for x in rs
                        if x["crawler_count"] == 0
                        or sum([y in x["category"] for y in bl]) == 0
                    ]
            except MySQLdb.OperationalError as e:
    ...
    def get_last_page(self, url_hash, publish_time, compared_cols="*"):
        ...
        table_names = [
            "page_" + publish_time[:7].replace("-", "_"),
    ...
    def insert_page(self, page_row, table_name=None, diff_vals=(1, None, None)):
        ...
        if table_name is None:
            table_name = (
                "page_"
                + page_row["publish_time"][:4]
                + "_"
                + page_row["publish_time"][5:7]
            )

        sql = (
            "INSERT INTO `"
            + table_name
            + "`(`list_id`, `url`, `url_md5`, `redirected_url`, `source_id`, \
